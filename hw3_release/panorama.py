...
def harris_corners(img, window_size=3, k=0.04):
    ...
    I_yy = dy * dy
    I_xx = dx * dx
    det = convolve(I_xx,window) * convolve(I_yy,window)
    det -= (convolve(I_xy,window) * convolve(I_xy,window))
    trace = convolve(I_xx,window) + convolve(I_yy,window)
...
def simple_descriptor(patch):
    ...
    feature = []
    ### YOUR CODE HERE
    assert(np.std(patch)!=0)
    tmp = (patch - np.mean(patch)) / np.std(patch)
    feature = tmp.flatten()
    pass
    ### END YOUR CODE
    return feature
...
def match_descriptors(desc1, desc2, threshold=0.5):
    ...
    M = desc1.shape[0]
    dists = cdist(desc1, desc2)#cdist为导入的计算欧氏距离的函数

    ### YOUR CODE HERE
    import heapq
    for i in range(M):
        tmp = heapq.nsmallest(2,dists[i,:])#该函数计算第二个参数中最小的第一个参数个元素
        if tmp[0] / tmp[1] < threshold:
            matches.append([i,np.argmin(dists[i,:])])#返回的是匹配的pair的索引对
    #如果直接返回matches会报错list没有shape属性，所以要转换成ndarray
...
def ransac(keypoints1, keypoints2, matches, n_iters=200, threshold=20):
    ...
    for i in range(n_iters):
        curInliers = np.zeros(N)
        #https://blog.csdn.net/autoliuweijie/article/details/51982514
        #https://numpy.org/devdocs/reference/random/generated/numpy.random.choice.html?highlight=choice#numpy.random.choice
        #https://blog.csdn.net/qw___/article/details/71173664
        indices = np.random.choice(N,n_samples,replace=False)
        points1 = matched1[indices,:]
        points2 = matched2[indices,:]
        
        #选择出的匹配点最小二乘
        H,r,rank,s = np.linalg.lstsq(points2, points1, rcond=None)
        H[:,2] = np.array([0, 0, 1])
        #计算inliers数量
        #参考：https://yongyuan.name/blog/fitting-line-with-ransac.html
        transformed = np.dot(matched2,H)
        err_per_point = np.sum((transformed-matched1)**2,axis=1) # sum squared error per row
        #curInliers实际上是一个bool的数组，用于筛选inliers索引
        curInliers = err_per_point < threshold
        #计算bool数组的sum，也就是true的数量，即inliers的数量
        inliersCnt = np.sum(curInliers)
        
        #更新
        if inliersCnt > n_inliers:
            max_inliers = curInliers.copy()
            n_inliers = inliersCnt
    #迭代结束，最小二乘估计
    H,r,rank,s = np.linalg.lstsq(matched2[max_inliers], matched1[max_inliers], rcond=None)
    H[:,2] = np.array([0, 0, 1])
    pass
    ### END YOUR CODE
    print(H)
        ...
        for col in range(cols):
            for i in range(pixels_per_cell[0]):
                for j in range(pixels_per_cell[1]):
                    #获取梯度方向
                    curTheta = theta_cells[row,col,i,j]
                    #计算在哪个区间
                    bin = int(curTheta) // degrees_per_bin
                    if bin == 9:
                        bin = 8
                    # 每个方向区间内统计
                    cells[row,col,bin] += G_cells[row,col,i,j]
    #归一化处理
    cells = (cells - np.mean(cells)) / cells.std()
    #转换为1D
