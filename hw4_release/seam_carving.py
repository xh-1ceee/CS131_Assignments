...
def compute_cost(image, energy, axis=1):
    ...
    ### YOUR CODE HERE
    #参考mikucy https://github.com/mikucy/CS131/blob/master/hw4_release/seam_carving.py
    #时限要求在0.05s内，一个循环实现，考虑把前一行cost进行操作，对于左右两个边界设置一个较大值
    for i in range(1,H):
        costL = np.insert(cost[i-1,0:W-1],0,1e10,axis=0)
        costM = cost[i-1,:]
        costR = np.insert(cost[i-1,1:W],W-1,1e10,axis=0)
        #将三个cost拼接并且reshape成每个cost是一行的形式
        costC = np.r_[costL,costM,costR].reshape(3,-1)
        cost[i] = energy[i] + np.min(costC,axis = 0)
        #path要求只能-1，0，1
        paths[i] = np.argmin(costC,axis = 0) - 1
    pass
    ### END YOUR CODE
...
def find_seams(image, k, axis=1, efunc=energy_function, cfunc=compute_cost, bfunc=backtrack_seam, rfunc=remove_seam):
    ...
    for i in range(k):
        ...
        #这里相对于原程序有修改
        indices = rfunc(indices, seam).astype(int)
...
def enlarge(image, size, axis=1, efunc=energy_function, cfunc=compute_cost, dfunc=duplicate_seam, bfunc=backtrack_seam, rfunc=remove_seam):
    ...
    ### YOUR CODE HERE
    seams_need = size - W
    seams = find_seams(out,seams_need)
    #先将seams转换为三维的
    seams = seams[:,:,np.newaxis]
    for i in range(seams_need):
        #find_seam返回的是一个存储每个位置的像素所属的seam序号（从1开始，0表示不在结果的seam当中）
        #where得到的结果是每个满足要求的位置索引，由于是列数的增加，只要相应第二维坐标即可
...
def compute_forward_cost(image, energy):
    ...
    for i in range(1,H):
        ...
        # I(i,j+1)
        IR = np.insert(image[i,0:W-1],0,0,axis = 0)
        # I(i-1.j)
        IM = image[i-1,:]
        # I(i,j-1)
        IL = np.insert(image[i,1:W],W-1,0,axis = 0)
        ...
        costC = np.r_[costL+CL,costM+CU,costR+CR].reshape(3,-1)
        cost[i] = energy[i] + np.min(costC,axis = 0)
        #path要求只能-1，0，1
        paths[i] = np.argmin(costC,axis = 0) - 1
    pass
